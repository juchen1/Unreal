...
## 언리얼 시작 열번째 날 date:24-08-25

with Unreal engine KR[러셀] - start Unreal 2022 lesson 5

### 시네 카메라 화면비율 변경하기
- 시네카메라에 우클릭  [필름백]을 설정해두면 비율을 고정함
- 시네마 시코브 종횡비설정 가능 - 전체 클릭으로 종횡비 설정가능

### 시퀀서 - 카메라 추가하기
- 레벨시퀀스 추가 / 저장 위치[Sequencer로 폴더 만들기] / 아웃라이너에 있는 시네마를 시퀀서에 넣어두기
- 카메라 아이콘을 눌러 파일럿 활성화 가능
- 카메라 컷[최종영상 컷] 이곳을 뷰포트 이동 불가
- 스냅으로 정확한 시간대로 설정 가능 / +카레마로 다른 카메라를 추가 가능
- 시간 단위 / 프레임프레임레이트 1초에 사진이 몇개 들어가는지. 일반적으로 1초에 30
- 한장면에 2초씩 설정하기
- 아래헤 프레임을 높일 수 있으음 / 영상길이는 빨간 줄로 / 마우스 휠로 확대축소가능

### 카메라 애니메이션 넣기
- 트랜스폼 / 조그만 동그라미 단추[키프레임***] 처음과 끝 키프레임을 자연스럽게 흐름 / 주의: 각 카메라에 키프레임 넣기
- 키프레임을 자동이 아닌 선형으로 변경. / 시퀀서 커브에서 확인 가능 / 키프레인을 클릭해서 뷰포트에서 원하는 시점으로 이동 가능
- 초점 길이는 조금짧게 해주기

### 시퀀서 - 라이트 내이메이션 추가하기
- 디렉셔널 라이트를 시퀀서에 넣어두기 / 트랜스폼 추가하기
- 중대하 이슈! / 한개의 디렉셔널로 키프레임을 설정하기 / 우클릭 - 프로퍼티 - 쿼터니온 보정 /
- 영상 파일로 추가하기 / 렌더링하기
...
### Q&A
- 콘텐츠 브라우저 정리 규칙
- 머티리얼, 스태틱, 텍스처 - 한 폴더에 넣어두기 혹은 각각 폴더에 넣어두기메가 스캔 외에도 다른 파일들이 있음
- 
